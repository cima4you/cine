...
# -*- coding: utf-8 -*-
"""
سكربت استخراج بيانات الأفلام من موقع tuktukhd.com
...
DEFAULT_CATEGORY = "https://tuktukhd.com/category/movies-2/%D8%A7%D9%81%D9%84%D8%A7%D9%85-%D8%A7%D8%AC%D9%86%D8%A8%D9%8A/"
...
def extract_movie_details(movie_url):
    ...
    download_servers = []
    for link in soup.select('.downloads a[href*="go.php"]'):
        href = extract_attr(link, 'href')
        name_el = link.select_one('span') or link.select_one('.download--item span')
        server_name = extract_text(name_el).strip() or 'Download'
        if href:
            ...
            if 'u' in qs:
                ...
                try:
                    ...
                    download_servers.append({'name': server_name, 'url': href})
            else:
                download_servers.append({'name': server_name, 'url': href})
...
def main():
    parser = argparse.ArgumentParser(
        description='🎬 سكربت استخراج ودمج أفلام من tuktukhd.com',
