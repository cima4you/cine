...
print('Scraping Turkish pages {} to {}...'.format(start_page, end_page))
...
def extract_details(text):
    ...
    # Categories (from breadcrumb, excluding nav links)
    cats = []
    seen_cats = set()
...
def extract(m):
    try:
        ...
        for k in ['توقيت الفيلم :', 'جودة الفيلم :', 'بطولة :', 'المخرجين :', 'دولة الفيلم :', 'لغة الفيلم :']:
            if det.get(k.replace(' :', '')) or det.get(k):
                v = det.get(k.replace(' :', '')) or det.get(k, '')
                if v.strip():
...
out_file = 'scripts/tuktukhd/data/results_turkish.json'
