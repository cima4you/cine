...
print('Total movies from index: {}'.format(len(all_movies)))
...
def extract_details(text):
    ...
    # Extract ACTUAL category (not breadcrumb)
    actual_cats = []
    m = re.search(r'التصنيفات\s*</span>\s*<a[^>]*>([^<]+)</a>', text)
    if m:
        actual_cats.append(m.group(1).strip())
    
    # Also get genres (الانواع)
    genres = []
    idx = text.find('الانواع')
    if idx > 0:
        sec = text[idx:idx+300]
        genres = re.findall(r'<a[^>]*href="[^"]*/genre/[^"]*"[^>]*>([^<]+)</a>', sec)
...
# Filter to ONLY anime movies
anime_only = [r for r in successful if any('انمي' in (c or '') for c in (r.get('category') or []))]
print('\nAnime movies (filtered): {}'.format(len(anime_only)))
print('Non-anime movies (excluded): {}'.format(len(successful) - len(anime_only)))

if not anime_only:
    print('No anime movies found! Saving all with category info for review.')
    anime_only = successful
