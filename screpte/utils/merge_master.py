...
INPUT_DIR = os.path.join(SCRIPT_DIR, 'scripts', 'tuktukhd', 'data')

# ✅ Pre-compile all regex patterns (HUGE speedup)
RE_SPACES = re.compile(r'\s+')
RE_YEAR = re.compile(r'20[12][0-9]')
RE_FILM = re.compile(r'^فيلم')
RE_MUSALSAL = re.compile(r'^مسلسل')
...
def build_norm_index(existing_data):
    """Pre-compute normalized titles for ALL existing data - do this ONCE."""
    norm_map = []
    norm_set = set()
...
def is_duplicate_fast(new_title, norm_map, norm_set, existing_data):
    ...
    # ✅ O(1) exact match check using set
    if new_norm in norm_set:
        for i, n in enumerate(norm_map):
            if n == new_norm:
                return existing_data[i]

    # Fuzzy "contains" check (only if exact match fails)
...
def transform_film(film, film_type):
    ...
    if isinstance(download_servers_raw, list):
        download_servers = [{"name": s.get('name', 'Download'), "url": s.get('url', '')} for s in download_servers_raw]
...
pattern = os.path.join(INPUT_DIR, 'results_master_*.json')
files = glob.glob(pattern)
files = [f for f in files if not f.endswith('results_master_all.json')]

if not files:
    print('No results_master_*.json files found in', INPUT_DIR)
    print('Run scrape_master.py first.')
    exit()
...
# ✅ Build the index ONCE (this is the key optimization)
...
for filepath in sorted(files):
    ...
    film_type = basename.replace('results_master_', '').replace('.json', '')
...
print('Usage: python scripts/utils/merge_master.py')